...
import { CriblMgmtPlaneDefaultError } from "../models/errors/criblmgmtplanedefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | CriblMgmtPlaneDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<
        T,
        E | CriblMgmtPlaneDefaultError | ResponseValidationError
      >,
      raw: unknown,
    ]
  > {
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new CriblMgmtPlaneDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new CriblMgmtPlaneDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
