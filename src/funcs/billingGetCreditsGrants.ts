 ...
 */
export function billingGetCreditsGrants(
  client: CriblMgmtPlaneCore,
  request: operations.V1BillingGetCreditsGrantsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.V1BillingGetCreditsGrantsResponse,
    | CriblMgmtPlaneError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.V1BillingGetCreditsGrantsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc(
    "/v1/organizations/{organizationId}/billing/credits/grants",
  )(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.V1BillingGetCreditsGrantsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail([503, "5XX"]),
    M.json(
      "default",
      operations.V1BillingGetCreditsGrantsResponse$inboundSchema,
    ),
  )(response, req);
  if (!result.ok) {
