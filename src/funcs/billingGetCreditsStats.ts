 ...
 */
export function billingGetCreditsStats(
  client: CriblMgmtPlaneCore,
  request: operations.V1BillingGetCreditsStatsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.V1BillingGetCreditsStatsResponse,
    | CriblMgmtPlaneError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.V1BillingGetCreditsStatsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc(
    "/v1/organizations/{organizationId}/billing/credits/stats",
  )(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.V1BillingGetCreditsStatsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail([503, "5XX"]),
    M.json(
      "default",
      operations.V1BillingGetCreditsStatsResponse$inboundSchema,
    ),
  )(response, req);
  if (!result.ok) {
